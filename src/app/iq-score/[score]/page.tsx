...
function getClassification(iq: number): { label: string; color: string; band: string } {
  if (iq >= 130) return { label: "Gifted", color: "#5B4FCF", band: "Top 2%" };
  if (iq >= 120) return { label: "Superior", color: "#4A3EBE", band: "Top 9%" };
  if (iq >= 110) return { label: "Above Average", color: "#6B5FD9", band: "Top 25%" };
  if (iq >= 90)  return { label: "Average", color: "#8B7FE8", band: "Middle 50%" };
  if (iq >= 80)  return { label: "Below Average", color: "#9896A8", band: "Bottom 25%" };
    ...
    <div style={{ backgroundColor: "#F7F6F2", minHeight: "100vh", fontFamily: "var(--font-body, sans-serif)" }}>
      ...
      <main style={{ maxWidth: "720px", margin: "0 auto", padding: "48px 24px 80px" }}>
        ...
        <nav style={{ fontSize: "12px", color: "#9896A8", marginBottom: "32px" }}>
          ...
          <Link href="/iq-score/100" style={{ color: "#9896A8", textDecoration: "none" }}>IQ Scores</Link>
          <span style={{ margin: "0 8px" }}>›</span>
          <span style={{ color: "#1A1825" }}>IQ {iq}</span>
        </nav>
        ...
        <div style={{ backgroundColor: "#fff", border: "1px solid #E8E5DC", borderRadius: "16px", padding: "32px", marginBottom: "16px" }}>
          ...
          {[
            ...
            <div key={row.range} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", borderRadius: "8px", marginBottom: "4px", backgroundColor: row.current ? "#EDE9FF" : "transparent", border: row.current ? "1px solid #C4BBFF" : "1px solid transparent" }}>
              ...
              {row.current && <span style={{ marginLeft: "8px", fontSize: "11px", fontWeight: 700, color: "#5B4FCF" }}>← You</span>}
            </div>
          ))}
        </div>
        ...
        <div style={{ backgroundColor: "#fff", border: "1px solid #E8E5DC", borderRadius: "16px", padding: "28px 32px", marginBottom: "32px" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
            {related.map(s => (
              <Link key={s} href={`/iq-score/${s}`} style={{ backgroundColor: "#F7F6F2", border: "1px solid #E8E5DC", color: "#5B4FCF", padding: "8px 16px", borderRadius: "8px", fontSize: "13px", fontWeight: 600, textDecoration: "none" }}>
                IQ {s}
              </Link>
            ))}
          </div>
        ...
        <div style={{ backgroundColor: "#1A1825", borderRadius: "16px", padding: "40px 32px", textAlign: "center" }}>
          ...
          <Link href="/test" style={{ display: "inline-block", backgroundColor: "#5B4FCF", color: "#fff", padding: "16px 40px", borderRadius: "8px", fontSize: "15px", fontWeight: 600, textDecoration: "none" }}>
